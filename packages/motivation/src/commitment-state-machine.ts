...
export class CommitmentStateMachine {
  private state: CommitmentState = 'active';
  private goalId: string;
  private transitionsCount = 0;
  private createdAt: number;
  private lastTransitionAt: number;
  private hooks: StateChangeHook[] = [];
  private autoRestoreBudget = false;
  private autoRestoreUsage = false;
  private budgetRestoreTimer?: NodeJS.Timeout;
  private usageRestoreTimer?: NodeJS.Timeout;

  constructor(goalId: string, config?: StateMachineConfig) {
    this.goalId = goalId;
   ...
   * 状态转换
   */
  transition(event: CommitmentEvent): boolean {
    ...
    const context: StateChangeContext = {
      goalId: this.goalId,
  ...
  private startBudgetRestoreTimer(context: StateChangeContext): void {
  ...
  private startUsageRestoreTimer(context: StateChangeContext): void {
 ...
 * 快速创建带钩子的状态机
 */
export function createMonitoredMachine(
  goalId: string,
  callbacks: {
    onBudgetExceeded?: (context: StateChangeContext) => void;
    onUsageLimited?: (context: StateChangeContext) => void;
    onStateChange?: (from: CommitmentState, to: CommitmentState, context: StateChangeContext) => void;
