# Medical Advisor 测试场景文档

> **版本**: 1.1.0（v3.6.0.1 去中康化版）  
> **日期**: 2026-06-14  
> **用途**: Medical Advisor 自动化测试用例  
...
```
1. dataSource.query('drug', { drugName: "阿莫西林" })
   → 调用药品数据库
...
```
1. dataSource.query('report', { reportData: {...} })
   → 调用体检报告解读接口
...
```
1. dataSource.query('drug', { drugName: "二甲双胍" })
2. dataSource.query('drug', { drugName: "格列美脲" })
3. 查询药物相互作用
   → 分析相互作用
```
...
```
1. dataSourceRegistry.resolve('literature')
   → 解析到合适的数据源（PubMed 或主数据源）
   
2. dataSource.query('literature', { query: "肺癌 免疫治疗", options: {...} })
   → 检索文献数据库
...
```
1. dataSource.query('qa', {
     symptoms: ["头晕"],
     context: { age: 65, medicalHistory: ["高血压", "糖尿病"] }
   })
...
```
1. dataSource.query('report', {
     condition: "高血压",
     patientProfile: { age: 58, gender: "male", weight: 80, height: 170 }
   })
...
| 版本 | 日期 | 更新内容 |
|------|------|----------|
| 1.1.0 | 2026-06-14 | v3.6.0.1 去中康化：更新数据源引用方式 |
