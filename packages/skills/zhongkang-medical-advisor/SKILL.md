# SKILL: Zhongkang Medical Advisor（中康医疗助手）
...
基于中康科技医疗数据库的智能医疗助手，整合 30万+ 药品说明书、19年医药产业数据、卓睦鸟医疗大模型，提供药品查询、疾病诊断参考、体检报告解读、科研文献检索、医疗数据分析等能力。本 Skill 是 SelfClaw v3.6.0 医疗垂类模板的标准化实现，可直接通过 skill-factory-core 生成定制化医疗 Skill。
...
- **药品数据库**：30万+ 药品说明书（商品名/通用名/别名）
- **疾病知识库**：ICD-10 疾病分类、临床指南、专家共识
- **体检标准**：各类体检指标参考范围
- **文献资源**：PubMed 摘要、临床试验数据
- **市场数据**：药品销量、市场份额、行业趋势
...
```yaml
name: zhongkang-medical-advisor
version: 1.0.0
risk_level: high
requires:
  - network:http
...
async function analyzeMedicalQuery(input: MedicalQuery): Promise<MedicalResponse> {
  ...
  if (input.intent === 'drug_query') {
    try {
      const drugInfo = await zhongkangAPI.getDrugInfo(input.drugName);
